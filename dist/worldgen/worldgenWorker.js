...
async function generateGeology(command) {
    ...
    try {
        ...
        const oceanicAgeMyr = output.oceanic_age_myr();
        const continentalBasementAgeMyr = output.continental_basement_age_myr();
...
async function generateClimate(command) {
    ...
    try {
        ...
        const bedrockClass = output.bedrock_class();
        const rockStrengthIndex = output.rock_strength_index();
        const lithologyErodibilityIndex = output.lithology_erodibility_index();
        const permeabilityIndex = output.permeability_index();
        const weatheringSusceptibility = output.weathering_susceptibility();
        const finesFraction = output.fines_fraction();
        const carbonateFraction = output.carbonate_fraction();
        ...
        const postInfillSolidElevationM = output.post_infill_solid_elevation_m();
        const lakeFillDepthM = output.lake_fill_depth_m();
        const result = {
            engineVersion: output.generator_version(), coarseLevel: output.coarse_level(), fineLevel: output.fine_level(),
            generationTimings: [],
            stage: { id: output.stage_id(), version: output.stage_version(), stageSeed: output.stage_seed_hex(), durationMs: Math.max(0, nowMs() - startedAt) },
...
workerScope.addEventListener('message', async (messageEvent) => {
    ...
    try {
        ...
        if (command.type === 'generate-synthetic') {
            ...
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-synthetic', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-topology') {
            const result = await generateTopology(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-topology', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-tectonics') {
            const result = await generateTectonics(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-tectonics', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-geology') {
            const result = await generateGeology(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-geology', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-lithosphere') {
            const result = await generateLithosphere(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-lithosphere', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-inheritance') {
            const result = await generateInheritance(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-inheritance', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-topography') {
            const result = await generateTopography(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-topography', payload: result }, collectTransferables(result));
            return;
        }
        if (command.type === 'generate-drainage') {
            const result = await generateDrainage(command);
            workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-drainage', payload: result }, collectTransferables(result));
            return;
        }
