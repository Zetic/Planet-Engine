...
function makePalette(field) {
    const palette = new Array(PALETTE_STEPS);
...
let cachedPalette = [];
function prepareStyles(result, mode) {
    ...
    if (mode === 'land-water') {
        ...
        for (let sample = 0; sample < samplePaletteIndices.length; sample += 1)
            samplePaletteIndices[sample] = result.submergedMask[sample] ? 0 : 1;
        return;
    }
    if (mode === 'plates') {
        cachedPalette = new Array(result.metrics.plateCount);
        for (let plate = 0; plate < cachedPalette.length; plate += 1)
            cachedPalette[plate] = `hsl(${(plate * 137.507764 + 18) % 360} 60% 55%)`;
        for (let sample = 0; sample < samplePaletteIndices.length; sample += 1)
            samplePaletteIndices[sample] = result.plateIds[sample];
        return;
    }
    const field = scalarField(result, mode) ?? scalarField(result, 'relative-elevation');
    cachedPalette = makePalette(field);
    for (let sample = 0; sample < samplePaletteIndices.length; sample += 1)
        samplePaletteIndices[sample] = paletteIndex(field.values[sample], field.minimum, field.maximum);
}
function renderPlanet(canvas, result, projection, mode, yaw, pitch, buffers, interactive) {
    ...
    for (let paletteIndexValue = 0; paletteIndexValue < cachedPalette.length; paletteIndexValue += 1) {
        ...
        if (!fastPoints)
            context.beginPath();
        for (let sample = 0; sample < count; sample += 1) {
            if (!buffers.visible[sample] || samplePaletteIndices[sample] !== paletteIndexValue)
                continue;
