...
let edgeOverlayCache = { result: null, coastline: new Uint32Array(0), contours: [] };
...
function drawDiagnosticOverlays(context, result, overlays, phase, projection, yaw, pitch, width, height, buffers, animation) {
    ...
    if (overlays.has('coastline'))
        strokeSamplePairs(context, edgeCache.coastline, buffers, projection, width, 'rgba(225,236,246,0.78)', 1.15);
...
async function generatePlanet() {
    ...
    try {
        ...
        if (loaded.erosionMetrics.seasonalHydrologyHash !== loaded.seasonalMetrics.seasonalHydrologyHash)
            throw new Error('WG-7A seasonal identity does not match accepted WG-6D hydrology.');
        ...
        if (loaded.evolutionMetrics.drainageHash !== loaded.drainageMetrics.drainageHash)
            throw new Error('WG-7B drainage identity does not match accepted WG-6A topology.');
        if (loaded.evolutionMetrics.runoffHash !== loaded.runoffMetrics.runoffHash)
            throw new Error('WG-7B runoff identity does not match accepted WG-6B runoff.');
        if (loaded.evolutionMetrics.lakeHash !== loaded.lakeMetrics.lakeHash)
            throw new Error('WG-7B lake identity does not match accepted WG-6C state.');
