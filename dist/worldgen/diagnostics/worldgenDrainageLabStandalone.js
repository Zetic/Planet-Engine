...
import { WORLDGEN_INVALID_SAMPLE_ID } from '../protocol.js';
const TWO_PI = Math.PI * 2;
const INVALID = WORLDGEN_INVALID_SAMPLE_ID;
function element(id) {
    const target = document.getElementById(id);
    if (!target)
        throw new Error(`WG-6A Drainage Lab is missing #${id}.`);
    return target;
}
...
function discreteColor(id, saturation = 62, lightness = 53) {
    return `hsl(${(id * 137.507764 + 32) % 360} ${saturation}% ${lightness}%)`;
}
function scalarColor(t, lowHue, highHue) {
...
function sampleColor(result, mode, sample) {
    if (mode === 'physical-elevation')
        return hypsometricColor(result, sample);
    if (mode === 'land-water')
        return result.submergedMask[sample] ? '#1f6795' : '#7aa05d';
    if (result.submergedMask[sample])
        return '#102c43';
...
function ensureBuffers(buffers, count) {
    if (buffers.x.length === count)
        return buffers;
...
function drawReceiverOverlay(context, result, projection, width, buffers) {
    ...
    for (let sample = 0; sample < result.metrics.sampleCount; sample += 1) {
        ...
        if (receiver === INVALID || !buffers.visible[receiver])
            continue;
...
function drawOutlets(context, result, buffers) {
    context.save();
    context.fillStyle = 'rgba(255,255,255,0.92)';
    for (const outlet of result.basinOutletSamples) {
        if (outlet === INVALID || !buffers.visible[outlet])
            continue;
        context.beginPath();
...
function render(canvas, result, projection, mode, yaw, pitch, buffers) {
    const width = 1100;
    ...
    if (!context)
        throw new Error('WG-6A Drainage Lab could not acquire a 2D canvas context.');
    ...
    if (mode === 'flow-direction')
        drawReceiverOverlay(context, result, projection, width, buffers);
    if (mode === 'basins')
        drawOutlets(context, result, buffers);
}
...
const seedInput = element('worldgen-seed');
const coarseInput = element('worldgen-coarse-level');
const levelInput = element('worldgen-level');
const platesInput = element('worldgen-plates');
const generateButton = element('worldgen-generate');
const status = element('worldgen-status');
const modeSelect = element('worldgen-visualization');
const projectionSelect = element('worldgen-projection');
const timer = element('worldgen-generation-timer');
let current = null;
let buffers = { x: new Float32Array(0), y: new Float32Array(0), visible: new Uint8Array(0) };
let yaw = -0.55;
let pitch = 0.2;
let dragging = false;
let lastX = 0;
...
canvas.addEventListener('pointermove', event => {
    if (!dragging || projectionSelect.value !== 'globe')
        return;
