...
function element(id) {
    ...
    if (!target)
        throw new Error(`Worldgen Lab is missing #${id}.`);
    return target;
}
...
function scalarColorFromT(t, lowHue, highHue) {
    const hue = lowHue + (highHue - lowHue) * t;
    return `hsl(${hue} 68% ${38 + t * 22}%)`;
}
function scalar(result, mode) {
    switch (mode) {
...
function sampleColor(result, mode, sample, scalarDescriptor) {
...
let projectionBuffers = { x: new Float32Array(0), y: new Float32Array(0), visible: new Uint8Array(0) };
let styleCache = { result: null, mode: '', sampleBuckets: [], boundaryBuckets: [] };
function ensureProjectionBuffers(sampleCount) {
...
function projectSamples(result, projection, yaw, pitch, width, height) {
...
function renderPlanet(canvas, result, projection, mode, yaw, pitch, interactive) {
    const width = 1100;
    ...
    if (mode === 'mesh') {
        ...
        for (let sample = 0; sample < result.metrics.fineSampleCount; sample += 1) {
            ...
            for (let cursor = result.neighborOffsets[sample]; cursor < result.neighborOffsets[sample + 1]; cursor += 1) {
                ...
                if (neighbor <= sample || !visible[neighbor])
                    continue;
                const bx = x[neighbor];
    ...
    if (boundaryMode) {
        ...
        for (const bucket of styleCache.boundaryBuckets) {
            ...
            for (let cursor = 0; cursor < bucket.indices.length; cursor += 1) {
                ...
                if (!visible[sampleA] || !visible[sampleB])
                    continue;
                const ax = x[sampleA];
                const bx = x[sampleB];
                if (projection === 'map' && Math.abs(ax - bx) > width / 2)
