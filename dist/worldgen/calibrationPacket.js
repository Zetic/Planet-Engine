import { WORLDGEN_CRUST_CONTINENTAL, WORLDGEN_INVALID_SAMPLE_ID, } from './protocol.js';
...
function topographySummary(result) {
    ...
    for (let sample = 0; sample < result.metrics.fineSampleCount; sample += 1) {
        if (result.submergedMask[sample])
            waterDepth.push(Math.max(0, result.waterDepthM[sample]));
        else
            land.push(result.elevationAboveSeaLevelM[sample]);
    }
