import {
  ...
  type WorldgenGeneratedTopographyEvent,
...
interface WasmClimate {
  ...
  orbital_phase_count(): number; global_solver_level(): number; global_solver_sample_count(): number; spinup_years(): number; spinup_converged(): boolean; convergence_temperature_rms_k(): number; mean_temperature_k(): number; minimum_temperature_k(): number; maximum_temperature_k(): number; mean_land_temperature_k(): number; mean_ocean_temperature_k(): number; mean_wind_speed_m_s(): number; maximum_wind_speed_m_s(): number; mean_surface_current_m_s(): number; maximum_surface_current_m_s(): number; ocean_divergence_residual_m_s(): number; mean_sea_surface_temperature_k(): number; mean_annual_precipitation_mm(): number; p95_annual_precipitation_mm(): number; global_evaporation_kg(): number; global_precipitation_kg(): number; moisture_budget_relative_error(): number; moisture_transport_limiter_fraction(): number; maximum_moisture_transport_substeps(): number; persistent_snow_area_fraction(): number; sea_ice_area_fraction(): number; final_temperature_rms_change_k(): number;
  ...
  positions(): Float64Array;  neighbor_offsets(): Uint32Array; neighbors(): Uint32Array;
  ...
  receiver(): Uint32Array;   basin_id(): Uint32Array; depression_id(): Uint32Array; hydrologic_escape_elevation_m(): Float32Array; depression_depth_m(): Float32Array; contributing_area_m2(): Float64Array; 
  basin_outlet_samples(): Uint32Array; basin_outlet_kinds(): Uint8Array; basin_areas_m2(): Float64Array; depression_floor_samples(): Uint32Array; depression_floor_elevations_m(): Float64Array; depression_spill_elevations_m(): Float64Array; depression_areas_m2(): Float64Array;
     runoff_hash_hex(): string; runoff_parameter_hash_hex(): string; runoff_climate_hash_hex(): string; runoff_drainage_hash_hex(): string;
  ...
  actual_evapotranspiration_mm(): Float32Array; local_runoff_mm(): Float32Array; runoff_fraction(): Float32Array;  potential_discharge_m3_s(): Float32Array;
  ...
  effective_discharge_m3_s(): Float32Array; channel_slope(): Float32Array; channel_width_m(): Float32Array; erodibility_index(): Float32Array;  incision_potential_m_per_year(): Float32Array; local_sediment_supply_kg_s(): Float32Array;  sediment_load_kg_s(): Float32Array; sediment_deposition_kg_s(): Float32Array;
...
interface WorldgenWorkerScope {
  ...
  postMessage(message: WorldgenGeneratedSyntheticEvent | WorldgenGeneratedTopologyEvent | WorldgenGeneratedTectonicsEvent | WorldgenGeneratedGeologyEvent | WorldgenGeneratedLithosphereEvent | WorldgenGeneratedInheritanceEvent | WorldgenGeneratedTopographyEvent | WorldgenGeneratedClimateEvent | WorldgenGeneratedDrainageEvent | WorldgenGenerationProgressEvent | WorldgenErrorEvent, transfer?: Transferable[]): void;
}
...
async function generateClimate(command: Extract<WorldgenCommand, { type: 'generate-climate' }>): Promise<WorldgenClimateResult> {
  ...
  try {
    progress('packaging', 17, 18, 0, 1);
    const positions = output.positions(); const neighborOffsets = output.neighbor_offsets(); const neighbors = output.neighbors();
    ...
    const receiver = output.receiver(); const basinId = output.basin_id(); const depressionId = output.depression_id(); const hydrologicEscapeElevationM = output.hydrologic_escape_elevation_m(); const depressionDepthM = output.depression_depth_m(); const contributingAreaM2 = output.contributing_area_m2(); const basinOutletSamples = output.basin_outlet_samples(); const basinOutletKinds = output.basin_outlet_kinds(); const basinAreasM2 = output.basin_areas_m2(); const depressionFloorSamples = output.depression_floor_samples(); const depressionFloorElevationsM = output.depression_floor_elevations_m(); const depressionSpillElevationsM = output.depression_spill_elevations_m(); const depressionAreasM2 = output.depression_areas_m2();
    const actualEvapotranspirationMm = output.actual_evapotranspiration_mm(); const localRunoffMm = output.local_runoff_mm(); const runoffFraction = output.runoff_fraction(); const potentialDischargeM3S = output.potential_discharge_m3_s();
    ...
    const lakeDepressionIds = output.lake_depression_ids(); const lakeKinds = output.lake_kinds(); const lakeSurfaceElevationsM = output.lake_surface_elevations_m(); const lakeAreasM2 = output.lake_areas_m2(); const lakeVolumesM3 = output.lake_volumes_m3(); const lakeOutflowsM3S = output.lake_outflows_m3_s(); const seasonalPhaseSnowStorageMm = output.seasonal_phase_snow_storage_mm(); const seasonalPhaseRealizedDischargeM3S = output.seasonal_phase_realized_discharge_m3_s(); const seasonalFlowPresenceFraction = output.seasonal_flow_presence_fraction(); const seasonalFlowRegime = output.seasonal_flow_regime();
    const effectiveDischargeM3S = output.effective_discharge_m3_s(); const channelSlope = output.channel_slope(); const channelWidthM = output.channel_width_m(); const erodibilityIndex = output.erodibility_index(); const incisionPotentialMPerYear = output.incision_potential_m_per_year(); const localSedimentSupplyKgS = output.local_sediment_supply_kg_s(); const sedimentLoadKgS = output.sediment_load_kg_s(); const sedimentDepositionKgS = output.sediment_deposition_kg_s();
    ...
    const result: WorldgenClimateResult = {
      ...
      stage: { id: output.stage_id(), version: output.stage_version(), stageSeed: output.stage_seed_hex(), durationMs: Math.max(0, nowMs() - startedAt) },
      metrics: {
        coarseSampleCount: output.coarse_sample_count(), fineSampleCount: output.fine_sample_count(), plateCount: output.plate_count(), fineBoundaryEdgeCount: output.fine_boundary_edge_count(), orbitalPhaseCount: output.orbital_phase_count(), globalSolverLevel: output.global_solver_level(), globalSolverSampleCount: output.global_solver_sample_count(), spinupYears: output.spinup_years(), spinupConverged: output.spinup_converged(), convergenceTemperatureRmsK: output.convergence_temperature_rms_k(),
      ...
      receiver, basinId, depressionId, hydrologicEscapeElevationM, depressionDepthM, contributingAreaM2, basinOutletSamples, basinOutletKinds, basinAreasM2, depressionFloorSamples, depressionFloorElevationsM, depressionSpillElevationsM, depressionAreasM2,
      ...
      actualEvapotranspirationMm, localRunoffMm, runoffFraction, potentialDischargeM3S,
      ...
      lakeId, lakeKind, lakeFraction, lakeDepthM, realizedDischargeM3S, lakeDepressionIds, lakeKinds, lakeSurfaceElevationsM, lakeAreasM2, lakeVolumesM3, lakeOutflowsM3S, seasonalStage: { id: output.seasonal_stage_id(), version: output.seasonal_stage_version(), stageSeed: output.seasonal_stage_seed_hex(), durationMs: 0 },
      ...
      seasonalPhaseSnowStorageMm, seasonalPhaseRealizedDischargeM3S, seasonalFlowPresenceFraction, seasonalFlowRegime, erosionStage: { id: output.erosion_stage_id(), version: output.erosion_stage_version(), stageSeed: output.erosion_stage_seed_hex(), durationMs: 0 },
      ...
      effectiveDischargeM3S, channelSlope, channelWidthM, erodibilityIndex, incisionPotentialMPerYear, localSedimentSupplyKgS, sedimentLoadKgS, sedimentDepositionKgS,
...
workerScope.addEventListener('message', async messageEvent => {
  ...
  try {
    ...
    if (command.type === 'generate-synthetic') { const result = await generateSynthetic(command); workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-synthetic', payload: result }, collectTransferables(result)); return; }
    if (command.type === 'generate-topology') { const result = await generateTopology(command); workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-topology', payload: result }, collectTransferables(result)); return; }
    if (command.type === 'generate-tectonics') { const result = await generateTectonics(command); workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-tectonics', payload: result }, collectTransferables(result)); return; }
    if (command.type === 'generate-geology') {
      const result = await generateGeology(command);
      workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-geology', payload: result }, collectTransferables(result)); return;
    }
    if (command.type === 'generate-lithosphere') {
      const result = await generateLithosphere(command);
      workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-lithosphere', payload: result }, collectTransferables(result)); return;
    }
    if (command.type === 'generate-inheritance') {
      const result = await generateInheritance(command);
      workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-inheritance', payload: result }, collectTransferables(result)); return;
    }
    if (command.type === 'generate-topography') {
      const result = await generateTopography(command);
      workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-topography', payload: result }, collectTransferables(result)); return;
    }
    if (command.type === 'generate-drainage') {
      const result = await generateDrainage(command);
      workerScope.postMessage({ protocolVersion: WORLDGEN_PROTOCOL_VERSION, requestId: command.requestId, type: 'generated-drainage', payload: result }, collectTransferables(result)); return;
    }
    if (command.type === 'generate-climate') {
      ...
      const transferBytes = transferables.reduce<number>((sum, item) => sum + (item instanceof ArrayBuffer ? item.byteLength : 0), 0);
      workerScope.postMessage({
