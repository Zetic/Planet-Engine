...
function hypsometricColor(result: WorldgenClimateResult, sample: number): string {
  if (result.submergedMask[sample]) {
    ...
    if (depth > 7_500) return '#092847';
    if (depth > 6_000) return '#0d335a';
    if (depth > 4_500) return '#123f6c';
    if (depth > 3_500) return '#194f7e';
    if (depth > 2_500) return '#205f8e';
    if (depth > 1_500) return '#286f9e';
    if (depth > 900) return '#347fae';
    if (depth > 500) return '#4d9fc3';
    if (depth > 250) return '#69b7cf';
    if (depth > 100) return '#87c9d8';
    if (depth > 50) return '#a4dce1';
    return '#b7e5e6';
  }
