...
function drainageSampleColor(result: WorldgenClimateResult, mode: string, sample: number): string {
  if (result.submergedMask[sample]) return '#102c43';
...
function drawDrainageOutlets(context: CanvasRenderingContext2D, result: WorldgenClimateResult, buffers: ProjectionBuffers): void {
  context.save();
...
function hypsometricColor(result: WorldgenClimateResult, sample: number, bucketed = false): string {
  if (result.submergedMask[sample]) {
    ...
    if (depth <= 50) return '#afe1e4';
    if (depth <= 75) return '#a4dce1';
    if (depth <= 100) return '#99d5df';
    if (depth <= 150) return '#87c9d8';
    if (depth <= 250) return '#76bfd3';
    if (depth <= 350) return '#69b7cf';
    if (depth <= 500) return '#5fa9c7';
    if (depth <= 700) return '#589fbd';
    if (depth <= 900) return '#5196b4';
    if (depth <= 1_200) return '#4b8dab';
    if (depth <= 1_500) return '#4585a3';
    if (depth <= 1_800) return '#407e9c';
    if (depth <= 2_200) return '#3b7795';
    if (depth <= 2_600) return '#37718f';
    if (depth <= 3_000) return '#336b89';
    if (depth <= 3_500) return '#306683';
    if (depth <= 4_000) return '#2d617e';
    if (depth <= 4_500) return '#2a5d79';
    if (depth <= 5_250) return '#285a76';
    if (depth <= 6_000) return '#265773';
    if (depth <= 6_750) return '#245570';
    if (depth <= 7_500) return '#22536e';
    return '#20516c';
  }
...
function sampleColor(result: WorldgenClimateResult, mode: string, sample: number, field: ScalarField | null, bucketed = false): string {
  ...
  if (mode === 'historical-origin') return historicalIdentityColor(result.originPlateIds[sample]!, 42);
  ...
  if (RECONCILIATION_MODES.has(mode) && result.submergedMask[sample]) return '#102c43';
  ...
  if (projection === 'globe') {
    ...
    if (gpuDrawn) {
      ...
      const needsProjectedDecoration = cpuOverlayCount > 0
        ...
        || isDrainageMode(mode)
        || boundaryMode;
      if (needsProjectedDecoration) ensureProjectedSamples(result, projection, yaw, pitch, width, height, buffers, zoom);
...
const diagnosticLegend = element<HTMLElement>('worldgen-diagnostic-legend');
const overlayLegends = element<HTMLElement>('worldgen-overlay-legends');
...
type LegendItem = { label: string; color: string; kind?: 'swatch' | 'line' | 'vector' };
...
const DIAGNOSTIC_SCALE_HELP: Record<string, string> = {
...
function diagnosticScaleMeaning(mode: string): string {
  const specific = DIAGNOSTIC_SCALE_HELP[mode];
  if (specific) return specific;
  const categorical = CATEGORICAL_LEGENDS[mode];
  if (categorical) return 'Colors identify discrete classes; they are categorical rather than an ordered numeric scale.';
  const unit = DIAGNOSTIC_UNITS[mode];
  if (unit) return 'Legend values are shown directly in ' + unit + '. Colors progress from the displayed minimum to maximum, with values outside the display range clamped to an endpoint color.';
  if (current) {
    const field = scalarField(current, mode, orbitalPhase());
    if (field) return 'The numeric legend runs from ' + formatLegendNumber(mode, field.minimum) + ' to ' + formatLegendNumber(mode, field.maximum) + '; larger legend values correspond to larger values of this diagnostic.';
  }
  return 'The legend shows the ordering used by this diagnostic; generate a planet to see any data-dependent numeric bounds.';
}
function addDiagnosticHelp(mode: string): void {
...
function addLegendSwatches(items: LegendItem[], target: HTMLElement = diagnosticLegend): void {
...
function addScalarLegendGradient(mode: string, field: ScalarField): void {
  const ramp = document.createElement('div');
  ramp.className = 'worldgen-legend-ramp';
  const stops: string[] = [];
    ...
    const colorAt = mode === 'plates' || mode === 'kinematic-domains'
      ...
      : mode === 'basins' || mode === 'flow-direction' || mode === 'depressions'
        ...
        : mode === 'provenance' || mode === 'boundary-provenance'
          ? provenanceColor
          : (id: number): string => historicalIdentityColor(id, mode === 'historical-fragments' ? 104 : mode === 'historical-current' ? 18 : mode === 'historical-provenance' ? 154 : 42);
...
type OverlayLegendDefinition = { description: string; items: () => LegendItem[]; note?: string };
const OVERLAY_LEGENDS: Record<string, OverlayLegendDefinition> = {
...
function refreshOverlayLegends(): void {
  ...
  for (const input of selected) {
    ...
    if (definition.note) { const note = document.createElement('small'); note.textContent = definition.note; card.append(note); }
...
async function generatePlanet(): Promise<void> {
  ...
  try {
    ...
    if (loaded.lakeMetrics.drainageHash !== loaded.drainageMetrics.drainageHash) throw new Error('Final lake drainage identity does not match canonical WG-7D drainage.');
    if (loaded.lakeMetrics.runoffHash !== loaded.runoffMetrics.runoffHash) throw new Error('Final lake runoff identity does not match canonical WG-7D runoff.');
    ...
    if (loaded.seasonalMetrics.drainageHash !== loaded.drainageMetrics.drainageHash) throw new Error('Final seasonal drainage identity does not match canonical WG-7D drainage.');
    if (loaded.seasonalMetrics.runoffHash !== loaded.runoffMetrics.runoffHash) throw new Error('Final seasonal runoff identity does not match canonical WG-7D runoff.');
    if (loaded.seasonalMetrics.lakeHash !== loaded.lakeMetrics.lakeHash) throw new Error('Final seasonal lake identity does not match canonical WG-7D lake state.');
    if (loaded.erosionMetrics.inheritanceHash !== loaded.metrics.inheritanceHash) throw new Error('WG-7A inheritance identity does not match accepted fine physical state.');
    ...
    if (loaded.erosionMetrics.drainageHash !== loaded.reconciliationMetrics.preErosionDrainageHash) throw new Error('WG-7A drainage identity does not match WG-7C pre-erosion WG-6A ancestry.');
    ...
    if (loaded.erosionMetrics.seasonalHydrologyHash !== loaded.reconciliationMetrics.preErosionSeasonalHash) throw new Error('WG-7A seasonal identity does not match WG-7C pre-erosion WG-6D ancestry.');
...
season.addEventListener('input', () => { updateSeasonLabel(); styleCache = { result: null, key: '', sampleBuckets: [], boundaryBuckets: [] }; gpuColorCache = { result: null, key: '', colors: new Uint8Array(0), alpha: 0.94 }; refreshDiagnosticLegend(); if (selectedTile !== null) inspectTile(selectedTile); redraw(false); });
