...
path = Path("rust/interlink-worldgen/src/erosion.rs")
text = path.read_text()
text = once(
    text,
...
text = once(
    ...
    "default channel geometry",
)
old = "            self.channelized_area_fraction,\n"
if text.count(old) != 2:
    raise SystemExit(f"parameter hash/validation: expected two targets, found {text.count(old)}")
text = text.replace(
    old,
...
text = once(
    ...
    "performance width output",
)
perf.write_text(text)

smoke = Path("scripts/check-wg7a-erosion.sh")
