...
test('WG-4 browser contract remains available under protocol v13', () => {
  ...
  assert.deepEqual(worldgenTopographyCommand(77, { seed: 'wg4', coarseLevel: 4, fineLevel: 6, plateCount: 18 }), { protocolVersion: 13, requestId: 77, type: 'generate-topography', payload: { seed: 'wg4', coarseLevel: 4, fineLevel: 6, plateCount: 18 } });
});

test('Planet Engine Lab keeps every WG-3.75 and WG-4 view cumulatively through WG-6C', () => {
  const html = fs.readFileSync('index.html', 'utf8');
  assert.match(html, /PLANET ENGINE · THROUGH WG-6C/);
  ...
  assert.match(html, /one generation runs the accepted topology, tectonic, geological, lithospheric, multiresolution inheritance, WG-4 topography, WG-5 coupled climate, WG-6A drainage topology, WG-6B annual runoff\/discharge, and WG-6C lake equilibrium/i);
