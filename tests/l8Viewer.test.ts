...
test('GPU dual-cell mesh triangulates one contiguous polygon per sample', () => {
  ...
  assert.equal(mesh.centerWeights.length, 16);
  assert.deepEqual(Array.from(mesh.centerWeights.slice(0, 4)), [1, 0, 0, 0]);
...
test('L8 lab keeps one continuous GPU cell surface through zoom and interaction', () => {
  ...
  assert.match(gpu, /centerWeightBuffer/);
  assert.match(gpu, /drawElements\(gl\.TRIANGLES/);
  assert.match(gpu, /fwidth\(vCenterWeight\)/);
  assert.match(gpu, /uBorderStrength/);
  assert.match(gpu, /uSelectedCell/);
