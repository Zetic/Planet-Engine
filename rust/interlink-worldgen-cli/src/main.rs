...
fn usage() -> &'static str {
    "interlink-worldgen-cli <generate|benchmark|topology|tectonics|geology|lithosphere|inheritance|topography|climate|profile> [--seed TEXT] [--width N] [--height N] [--iterations N] [--level N] [--coarse-level N] [--plates N]"
}
...
fn climate(options: &Options) -> Result<(), String> {
    if options.coarse_level > options.level {
        return Err("--coarse-level cannot exceed --level".to_owned());
    }
    let started = Instant::now();
