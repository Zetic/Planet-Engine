...
fn wet_share(climate: &interlink_worldgen::ClimateState, terrain: &interlink_worldgen::TopographyState, topo: &interlink_worldgen::GeodesicTopology, threshold: f64) -> (f64, f64) {
    ...
    for i in 0..topo.metrics().sample_count as usize {
        ...
        land_area += a;
        total_p += p * a;
        if p >= threshold { wet_area += a; wet_p += p * a; }
    }
...
fn main() -> Result<(), String> {
    ...
    let plates = 16_u16;
    let scenarios = [
        ("base", 0.80_f64, 0.72_f64),
        ("eff055", 0.80, 0.55),
        ("eff040", 0.80, 0.40),
        ("rh085", 0.85, 0.72),
        ("rh090", 0.90, 0.72),
        ("rh085_e055", 0.85, 0.55),
        ("rh090_e050", 0.90, 0.50),
        ("rh090_e035", 0.90, 0.35),
    ];
    ...
    for seed in SEEDS {
        ...
        println!("SEED {seed}");
        for (name, rh, eff) in scenarios {
            let mut req = ClimateRequest::new(*seed);
            req.parameters.condensation_relative_humidity = rh;
            req.parameters.condensation_efficiency = eff;
            let climate = generate_coupled_climate(&fine, &terrain, planet, &req).map_err(|e| format!("{name}: {e}"))?;
            ...
            println!(
                "  {name:<12} rh={rh:.2} eff={eff:.2} P={:.0} p50={:.0} p95={:.0} cv={:.2} PET={:.0} AET={:.0} R={:.0} runoff={:.1}% wet1_area={:.1}% wet1_P={:.1}% wet2_area={:.1}% wet2_P={:.1}% substeps={} limiter={:.5}% mb={:.1e}",
                report.mean_land_precipitation_mm, report.land_precipitation_p50_mm,
                report.land_precipitation_p95_mm, report.land_precipitation_spatial_cv,
                report.mean_land_potential_evaporation_mm,
                runoff.metrics.mean_land_actual_evapotranspiration_mm,
                runoff.metrics.mean_land_runoff_mm, runoff.metrics.land_runoff_fraction * 100.0,
                wet1_area * 100.0, wet1_p * 100.0, wet2_area * 100.0, wet2_p * 100.0,
                climate.metrics.maximum_moisture_transport_substeps,
                climate.metrics.moisture_transport_limiter_fraction * 100.0,
