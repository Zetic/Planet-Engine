...
fn main() -> Result<(), String> {
    ...
    println!(
        "WG-3.6 tectonic orogens: provinces={} collision={} plateau={} cordilleran={} island_arc={} terrane={} transpressional={} coverage={:.1}% width={:.0}/{:.0}/{:.0}km core={} displaced_core={} runaway_core={} max_shortening={:.3} max_intensity={:.3} hash={}",
        ...
        displaced_core_samples,
        runaway_core_samples,
        orogens.metrics.maximum_shortening_index,
        orogens.metrics.maximum_orogenic_intensity,
        orogens.metrics.province_hash_hex(),
    );

    if mountain_core_samples == 0 || displaced_core_samples == 0 || runaway_core_samples != 0 {
        return Err("structural-strand localization invariant failed".to_string());
    }
