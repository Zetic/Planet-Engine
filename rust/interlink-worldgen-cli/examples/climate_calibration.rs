...
fn parse_options() -> Result<Options, String> {
    ...
    Ok(Options {
        ...
        skip_orography_intervention: args.iter().any(|arg| arg == "--skip-orography-intervention"),
    })
}
...
fn main() -> Result<(), String> {
    ...
    println!("seed={} coarse_level={} fine_level={} plates={}", options.seed, options.coarse_level, options.level, options.plates);
    println!("samples={} phases={} climate_hash={}", report.sample_count, report.orbital_phase_count, climate.metrics.climate_hash_hex());
    println!("temperature_k global={:.3} land={:.3} ocean={:.3} contrast={:.3} sst={:.3}", report.mean_temperature_k, report.mean_land_temperature_k, report.mean_ocean_temperature_k, report.land_ocean_temperature_contrast_k, report.mean_sea_surface_temperature_k);
    ...
    println!("clear_surface_asr_w_m2 global={:.3} land={:.3} ocean={:.3}", report.clear_surface_absorbed_shortwave_w_m2, report.clear_surface_absorbed_shortwave_land_w_m2, report.clear_surface_absorbed_shortwave_ocean_w_m2);
    println!("olr_proxy_w_m2 global={:.3} land={:.3} ocean={:.3} imbalance_proxy={:.3}", report.outgoing_longwave_proxy_w_m2, report.outgoing_longwave_land_proxy_w_m2, report.outgoing_longwave_ocean_proxy_w_m2, report.toa_energy_imbalance_proxy_w_m2);
    println!("transport_caps reconstructed_wind={:.6} reconstructed_moisture_edge={:.6}", report.reconstructed_wind_cap_fraction, report.reconstructed_moisture_edge_cap_fraction);
    println!("relative_humidity_proxy p05={:.6} p50={:.6} p95={:.6}", report.mean_state_relative_humidity_p05, report.mean_state_relative_humidity_p50, report.mean_state_relative_humidity_p95);
    println!("hydrology_mm_year precip_mean={:.3} precip_p95={:.3} pet_mean={:.3} p_over_e={:.6}", report.mean_annual_precipitation_mm, report.p95_annual_precipitation_mm, report.mean_potential_evaporation_mm, report.precipitation_to_evaporation_ratio);
    if let (Some(no_orography), Some(fraction)) = (report.no_orography_mean_annual_precipitation_mm, report.orographic_precipitation_causal_fraction) {
        println!("orography_intervention no_orography_precip_mean_mm={:.3} causal_fraction={:.6}", no_orography, fraction);
    }
    ...
    println!("ocean_heat annual_mean_tendency_rms_index={:.6}", report.annual_mean_ocean_heat_tendency_rms_index);
    ...
    for band in &report.latitude_bands {
        let sst = band.mean_sea_surface_temperature_k.map(|value| format!("{value:.3}")).unwrap_or_default();
        println!("latitude_band,{:.0},{:.0},{:.8},{:.3},{:.3},{:.8},{:.6},{},{:.3},{:.6}", band.minimum_latitude_deg, band.maximum_latitude_deg, band.area_fraction, band.mean_temperature_k, band.mean_precipitation_mm, band.mean_specific_humidity, band.mean_relative_humidity_proxy, sst, band.mean_snowfall_mm, band.mean_sea_ice_potential);
    }
