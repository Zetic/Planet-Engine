...
fn main() -> Result<(), String> {
    ...
    let fine_level = 7_u8;
    let plates = 16_u16;
    let speeds = [1.0_f64, 1.5, 2.0, 2.5, 3.0];
    ...
    for seed in SEEDS {
        ...
        for speed in speeds {
            ...
            req.parameters.maximum_climatological_moisture_transport_speed_m_s = speed;
            let climate = generate_coupled_climate(&fine, &terrain, planet, &req).map_err(|e| format!("speed {speed}: {e}"))?;
            ...
            println!(
                "  speed={speed:.1} P={:.0} p50={:.0} p95={:.0} cv={:.2} PET={:.0} AET={:.0} R={:.0} runoff={:.1}% wet1_area={:.1}% wet1_P={:.1}% wet2_area={:.1}% wet2_P={:.1}% substeps={} limiter={:.5}% mb={:.1e}",
