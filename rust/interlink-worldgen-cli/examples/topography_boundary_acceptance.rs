...
impl BoundaryStats {
    ...
    fn mean_submerged_depth_m(self) -> f64 {
        if self.submerged_endpoints == 0 {
            0.0
        } else {
            self.submerged_depth_sum_m / self.submerged_endpoints as f64
        }
    }
}
...
fn main() -> Result<(), String> {
    ...
    println!(
        "WG-4 boundary acceptance: land={:.3}% mean_land={:.1}m mean_ocean_depth={:.1}m ridge_land={:.2}% ridge_both_land={:.2}% ridge_submerged_depth={:.1}m max_seed_ridge_land={:.2}% max_seed_ridge_both_land={:.2}% collision_land={:.2}%",
