use interlink_worldgen::{
    ...
    GeologyRequest, LakeRequest, LithosphereRequest, PlanetPhysicalParameters, RunoffRequest,
...
fn parse_options() -> Result<Options, String> {
    ...
    let seed = args
        ...
        .cloned()
        .unwrap_or_else(|| "ci-wg6c-lakes".to_owned());
    let options = Options {
        seed,
...
fn main() -> Result<(), String> {
    ...
    .map_err(|error| error.to_string())?;
    let request = LakeRequest::new(options.seed.as_str());
    ...
    println!(
        "lakes total={} endorheic={} overflowing={} terminal_storage={} lake_samples={} area_km2={:.3} volume_km3={:.3} max_depth_m={:.3}",
        lakes.metrics.lake_count,
        lakes.metrics.endorheic_lake_count,
        lakes.metrics.overflowing_lake_count,
        lakes.metrics.terminal_storage_lake_count,
        lakes.metrics.lake_sample_count,
        lakes.metrics.total_lake_area_m2 / 1.0e6,
        lakes.metrics.total_lake_volume_m3 / 1.0e9,
        lakes.metrics.maximum_lake_depth_m,
    );
    println!(
        "water_balance lake_precip_m3_s={:.6} lake_evap_m3_s={:.6} terminal_realized_m3_s={:.6} storage_m3_s={:.6} relative_error={:.3e}",
        lakes.metrics.total_lake_precipitation_m3_s,
        lakes.metrics.total_lake_evaporation_m3_s,
        lakes.metrics.terminal_realized_discharge_m3_s,
        lakes.metrics.unreleased_storage_m3_s,
        lakes.metrics.water_balance_relative_error,
    );
    println!(
        "discharge potential_max_m3_s={:.6} realized_max_m3_s={:.6}",
        runoff.metrics.maximum_potential_discharge_m3_s,
        lakes.metrics.maximum_realized_discharge_m3_s,
    );
    println!(
        "hash lakes={} runoff={} drainage={} climate={} parameters={}",
        lakes.metrics.lake_hash_hex(),
        lakes.metrics.runoff_hash_hex(),
        lakes.metrics.drainage_hash_hex(),
        lakes.metrics.climate_hash_hex(),
        lakes.metrics.lake_parameter_hash_hex(),
    );
    Ok(())
}
