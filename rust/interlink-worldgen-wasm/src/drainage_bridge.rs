use interlink_worldgen::{
    build_icosphere, generate_drainage_topology, generate_historical_frontend,
    generate_initial_topography, generate_lithosphere_from_history, inherit_boundary_interfaces,
    inherit_physical_state, DrainageRequest, DrainageState, GeodesicTopology,
    HistoricalLithosphereRequest, LithosphereRequest, PlanetPhysicalParameters,
...
impl WasmWorldgenDrainage {
    ...
    pub fn stage_id(&self) -> String { self.drainage.stage.id.to_owned() }
    pub fn stage_version(&self) -> u32 { self.drainage.stage.version }
    pub fn stage_seed_hex(&self) -> String { format!("{:016x}", self.drainage.stage.derived_seed) }
    pub fn coarse_level(&self) -> u8 { self.coarse_level }
    pub fn fine_level(&self) -> u8 { self.topology.level() }
    pub fn sample_count(&self) -> u32 { self.drainage.metrics.sample_count }
    ...
    pub fn land_sample_count(&self) -> u32 { self.drainage.metrics.land_sample_count }
    pub fn ocean_sample_count(&self) -> u32 { self.drainage.metrics.ocean_sample_count }
    pub fn basin_count(&self) -> u32 { self.drainage.metrics.basin_count }
    pub fn depression_count(&self) -> u32 { self.drainage.metrics.depression_count }
    pub fn depression_sample_count(&self) -> u32 { self.drainage.metrics.depression_sample_count }
    pub fn land_area_m2(&self) -> f64 { self.drainage.metrics.land_area_m2 }
    pub fn terminal_contributing_area_m2(&self) -> f64 { self.drainage.metrics.terminal_contributing_area_m2 }
    pub fn area_conservation_relative_error(&self) -> f64 { self.drainage.metrics.area_conservation_relative_error }
    ...
    pub fn basin_outlet_samples(&self) -> Vec<u32> {
        self.drainage.basins.iter().map(|basin| basin.outlet_sample).collect()
    }
    pub fn basin_outlet_kinds(&self) -> Vec<u8> {
        self.drainage.basins.iter().map(|basin| basin.outlet_kind).collect()
    }
    ...
    pub fn depression_floor_samples(&self) -> Vec<u32> {
        self.drainage.depressions.iter().map(|depression| depression.floor_sample).collect()
    }
    pub fn depression_floor_elevations_m(&self) -> Vec<f64> {
        self.drainage.depressions.iter().map(|depression| depression.floor_elevation_m).collect()
    }
    pub fn depression_spill_elevations_m(&self) -> Vec<f64> {
        self.drainage.depressions.iter().map(|depression| depression.spill_elevation_m).collect()
    }
    pub fn depression_areas_m2(&self) -> Vec<f64> {
        self.drainage.depressions.iter().map(|depression| depression.area_m2).collect()
    }
}
