...
fn hash_f32(mut hash: u64, values: &[f32]) -> u64 {
    ...
    for value in values { hash = fnv_update(hash, &value.to_bits().to_le_bytes()); }
    hash
}
...
fn event_driven_hash(base_hash: u64, morphology_hash: u64, model: &OrogenProvinceModel) -> u64 {
    ...
    for values in [
        &model.orogenic_intensity, &model.mountain_core_index, &model.crustal_root_index,
        &model.plateau_index, &model.fold_thrust_index, &model.foreland_basin_index,
        &model.suture_index, &model.transpression_index, &model.maturity_index,
        &model.shortening_index, &model.local_width_km,
...
pub fn generate_event_driven_orogen_provinces<T: PlanetTopology>(
    ...
    let mut model = generate_tectonic_orogen_provinces(
        topology, tectonics, active_history, geology, pre, request, planet,
    )?;
    let count = topology.sample_count() as usize;
    let fields = [
        morphology.active_orogen_intensity.len(), morphology.fossil_orogen_intensity.len(),
        morphology.suture_intensity.len(), morphology.suture_age_myr.len(),
        morphology.shear_intensity.len(), morphology.accretion_intensity.len(),
    ...
    for sample in 0..count {
        ...
        let maturity = if age >= 0.0 { clamp01(0.25 + age / 280.0) } else { 0.0 };

        model.orogenic_intensity[sample] = f64::from(model.orogenic_intensity[sample])
            .max(active * 0.76)
            .max(fossil * (0.44 + 0.20 * shortening))
            .clamp(0.0, 1.0) as f32;
        model.suture_index[sample] = f64::from(model.suture_index[sample]).max(suture).clamp(0.0, 1.0) as f32;
        model.transpression_index[sample] = f64::from(model.transpression_index[sample])
            .max(shear * (0.38 + 0.42 * fossil)).clamp(0.0, 1.0) as f32;
        ...
        if fossil >= 0.20 {
            ...
            model.mountain_core_index[sample] = f64::from(model.mountain_core_index[sample]).max(core).clamp(0.0, 1.0) as f32;
            model.crustal_root_index[sample] = f64::from(model.crustal_root_index[sample]).max(root).clamp(0.0, 1.0) as f32;
            model.plateau_index[sample] = f64::from(model.plateau_index[sample]).max(plateau).clamp(0.0, 1.0) as f32;
            model.fold_thrust_index[sample] = f64::from(model.fold_thrust_index[sample]).max(fold).clamp(0.0, 1.0) as f32;
            model.foreland_basin_index[sample] = f64::from(model.foreland_basin_index[sample]).max(foreland).clamp(0.0, 1.0) as f32;
            model.maturity_index[sample] = f64::from(model.maturity_index[sample]).max(maturity * fossil).clamp(0.0, 1.0) as f32;
            model.shortening_index[sample] = f64::from(model.shortening_index[sample]).max(shortening * fossil).clamp(0.0, 1.0) as f32;
            model.local_width_km[sample] = f64::from(model.local_width_km[sample])
                .max(65.0 + 330.0 * fossil + 130.0 * suture).min(850.0) as f32;
    ...
    let total_area = (0..topology.sample_count()).map(|sample| topology.area_steradians(sample)).sum::<f64>().max(1.0e-12);
    let orogenic_area = (0..topology.sample_count())
        .filter(|sample| model.orogenic_intensity[*sample as usize] >= 0.10)
        .map(|sample| topology.area_steradians(sample)).sum::<f64>();
    model.metrics.orogenic_area_fraction = orogenic_area / total_area;
    model.metrics.maximum_orogenic_intensity = model.orogenic_intensity.iter().map(|value| f64::from(*value)).fold(0.0_f64, f64::max);
    model.metrics.maximum_shortening_index = model.shortening_index.iter().map(|value| f64::from(*value)).fold(0.0_f64, f64::max);
    model.metrics.province_hash = event_driven_hash(base_hash, morphology.metrics.morphology_hash, &model);
    Ok(model)
}
...
mod tests {
    ...
    fn fossil_orogens_survive_away_from_present_boundaries() {
        ...
        let frontend = generate_historical_frontend(
            &topology, &HistoricalLithosphereRequest::new(seed, 16), planet,
        ).unwrap();
        let active_history = generate_tectonic_history(
            &topology, &frontend.tectonics, &TectonicHistoryRequest::new(seed), planet,
        ).unwrap();
        let morphology = build_historical_tectonic_morphology(
            &topology, &frontend.historical, &frontend.tectonics, seed,
        ).unwrap();
        let pre = generate_pre_orogenic_lithosphere_from_history(
            &topology, &frontend.tectonics, &active_history, &frontend.geology, &morphology,
            &PreOrogenicLithosphereRequest::new(seed),
        ).unwrap();
        let model = generate_event_driven_orogen_provinces(
            &topology, &frontend.tectonics, &active_history, &frontend.geology, &pre, &morphology,
            &OrogenProvinceRequest::new(seed), planet,
        ).unwrap();
        ...
        let internal_fossil_relief = model.orogenic_intensity.iter().enumerate()
            .filter(|(sample, value)| **value >= 0.16 && !boundary_mask[*sample]).count();
