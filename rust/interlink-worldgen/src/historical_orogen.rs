...
pub fn generate_event_driven_orogen_provinces<T: PlanetTopology>(
    ...
    for sample in 0..count {
        ...
        // Preserve current active subduction/collision provinces while allowing fossil material
        // structure to remain an independent source of relief wholly inside a modern plate.
        model.orogenic_intensity[sample] = f64::from(model.orogenic_intensity[sample])
            .max(active * 0.76)
            .max(fossil * (0.48 + 0.22 * shortening))
        ...
        if fossil >= 0.16 {
            ...
            let core = fossil
                * (0.22 + 0.58 * suture + 0.24 * shear)
                * (0.36 + 0.64 * shortening.max(0.18))
                * (0.72 + 0.28 * inherited_weakness);
            let root = fossil * (0.24 + 0.54 * shortening + 0.22 * suture);
            let plateau = fossil * root * (0.18 + 0.34 * maturity);
            let fold = fossil * (0.22 + 0.52 * shortening + 0.18 * accretion);
            let foreland = fossil * fold * 0.22;
            ...
            model.local_width_km[sample] = f64::from(model.local_width_km[sample])
                .max(70.0 + 360.0 * fossil + 140.0 * suture)
                .min(900.0) as f32;

            // A fossil-only sample must be treated as continental/terrane/transpressional relief,
            // never as an active volcanic arc simply because a current boundary is nearby.
            if model.province_kind[sample] == 0 {
    ...
    model.metrics.maximum_shortening_index = model
        ...
        .map(|value| f64::from(*value))
        .fold(0.0_f64, f64::max);
    model.metrics.province_hash = event_driven_hash(
        base_hash,
