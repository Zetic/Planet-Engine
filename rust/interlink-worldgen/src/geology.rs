...
pub const GEOLOGY_STAGE_VERSION: u32 = 4;
...
fn weighted_influence_field<T: PlanetTopology>(
    topology: &T,
    source_strength: &[f64],
    scale_rad: f64,
) -> Vec<f32> {
...
fn orogenic_influence_field<T: PlanetTopology>(
    ...
    let mut register = |sample: u32, convergence: f64, collision: bool| {
        ...
        expected_neighbors[index] =
            expected_neighbors[index].max(if collision { 3.0 } else { 2.0 });
    };
...
fn history_fields<T: PlanetTopology>(
    ...
    let mut orogen = orogenic_influence_field(
        topology,
        kinds,
        ages,
        boundaries,
